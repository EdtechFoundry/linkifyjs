...
function stateify$$module$lib$linkify$core$state(a, b, c, e) {
  for (var f = 0, g = a.length, h = [], d = void 0;f < g && (d = b.next(a[f]));) {
    b = d, f++;
  }
  if (f >= g) {
    return [];
  ...
  for (;f < g - 1;) {
    d = new CharacterState$$module$lib$linkify$core$state(e), h.push(d), b.on(a[f], d), b = d, f++;
  }
  d = new CharacterState$$module$lib$linkify$core$state(c);
  h.push(d);
  b.on(a[g - 1], d);
  return h;
}
...
var MultiToken$$module$lib$linkify$core$tokens = function() {
  ...
  _inherits$$module$lib$linkify$core$tokens(b, a);
  b.prototype.toHref = function() {
    for (var a = 0 >= arguments.length || void 0 === arguments[0] ? "http" : arguments[0], b = !1, f = !1, g = this.v, h = [], d = 0;g[d] instanceof PROTOCOL$$module$lib$linkify$core$tokens;) {
      b = !0, h.push(g[d].toString().toLowerCase()), d++;
    }
    for (;g[d] instanceof SLASH$$module$lib$linkify$core$tokens;) {
      f = !0, h.push(g[d].toString()), d++;
    }
    for (;isDomainToken$$module$lib$linkify$core$tokens(g[d]);) {
...
var run$$module$lib$linkify$core$parser = function(a) {
  for (var b = a.length, c = 0, e = [], f = [];c < b;) {
    for (var g = S_START$$module$lib$linkify$core$parser, h = null, d = null, k = 0, l = null, m = -1;c < b && !(h = g.next(a[c]));) {
      f.push(a[c++]);
    }
    for (;c < b && (d = h || g.next(a[c]));) {
    ...
    if (0 > m) {
      ...
      0 < f.length && (e.push(new T_TEXT$$module$lib$linkify$core$parser(f)), f = []), c -= m, k -= m, g = l.emit(), e.push(new g(a.slice(c - k, c)));
    }
  }
  0 < f.length && e.push(new T_TEXT$$module$lib$linkify$core$parser(f));
  return e;
...
var run$$module$lib$linkify$core$scanner = function(a) {
  for (var b = a.toLowerCase(), c = a.length, e = 0, f = [];e < c;) {
    for (var g = S_START$$module$lib$linkify$core$scanner, h = null, d = 0, k = null, l = -1;e < c && (h = g.next(b[e]));) {
      g = h, g.accepts() ? (l = 0, k = g) : 0 <= l && l++, d++, e++;
    }
    0 > l || (e -= l, d -= l, g = k.emit(), f.push(new g(a.substr(e - d, d))));
  }
  return f;
...
function resolve$$module$lib$linkify$utils$options(a) {
  for (var b = arguments.length, c = Array(1 < b ? b - 1 : 0), e = 1;e < b;e++) {
    c[e - 1] = arguments[e];
  }
...
var tokenize$$module$lib$linkify = function(a) {
  ...
  for (var b = 1 >= arguments.length || void 0 === arguments[1] ? null : arguments[1], c = tokenize$$module$lib$linkify(a), e = [], f = 0;f < c.length;f++) {
    !c[f].isLink || b && c[f].type !== b || e.push(c[f].toObject());
  }
  return e;
